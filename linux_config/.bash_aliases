...
([ "$(hostname)" != "kstation" ] && ping -4 -c 1 gitee.com &> /dev/null && for configf in ".bash_aliases" ".vimrc" ".tmux.conf" ".config/htop/htoprc"; do mkdir -p `dirname ~/.kai_config/$configf`; mkdir -p `dirname ~/$configf`; wget -O ~/.kai_config/$configf https://gitee.com/kaiwu-astro/garage/raw/main/linux_config/$configf &> /dev/null; cp -p ~/.kai_config/$configf ~/$configf; done &)
